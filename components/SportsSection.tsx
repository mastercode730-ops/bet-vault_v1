const WHATSAPP_URL = `https://wa.me/918764465110?text=${encodeURIComponent('Hi Bet Vault! Can I get more info on this?')}`;

const sports = [
  { icon: '🏏', name: 'Cricket' },
  { icon: '⚽', name: 'Football' },
  { icon: '🏀', name: 'Basketball' },
  { icon: '🎾', name: 'Tennis' },
  { icon: '🏒', name: 'Hockey' },
  { icon: '🎰', name: 'Casino' },
  { icon: '🃏', name: 'Teen Patti' },
  { icon: '🎲', name: 'Live Casino' },
];

export default function SportsSection() {
  return (
    <section className="py-14 sm:py-20 bg-black" id="sports">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">

        {/* ── Section Header ── */}
        <div className="flex flex-col items-center text-center mb-10 sm:mb-14">
          <div className="inline-flex items-center gap-2 bg-yellow-600/10 border border-yellow-600/30 rounded-full px-4 py-2 mb-4">
            <span className="text-yellow-400 text-xs font-semibold uppercase tracking-wider">
              All Markets in One Place
            </span>
          </div>
          <h2
            className="font-black text-white mb-3 w-full"
            style={{ fontFamily: 'Montserrat, sans-serif', fontSize: 'clamp(1.4rem, 4vw, 2.4rem)', lineHeight: 1.2 }}
          >
            Bet on Whatever{' '}
            <span className="gold-text">Excites You</span>
          </h2>
          <p className="text-gray-400 text-sm sm:text-base max-w-xl">
            Cricket is our specialty, but that&apos;s just the beginning. One Bet Vault ID unlocks
            dozens of sports and casino games — all in one place.
          </p>
        </div>

        {/* ── Sports Grid ── */}
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 sm:gap-4 mb-8 sm:mb-12">
          {sports.map((sport, idx) => (
            <a
              key={idx}
              href={WHATSAPP_URL}
              target="_blank"
              rel="noopener noreferrer"
              id={`sport-${sport.name.toLowerCase().replace(' ', '-')}`}
              className="card-glow rounded-xl sm:rounded-2xl p-4 sm:p-6 flex flex-col items-center justify-center gap-2 sm:gap-3 group min-h-[90px] sm:min-h-[110px]"
            >
              <span className="text-3xl sm:text-4xl group-hover:scale-110 transition-transform duration-300 leading-none">
                {sport.icon}
              </span>
              <span className="text-white font-semibold text-xs sm:text-sm group-hover:text-yellow-400 transition-colors text-center leading-tight">
                {sport.name}
              </span>
            </a>
          ))}
        </div>

        {/* ── CTA Banner ── */}
        <div
          className="relative rounded-2xl sm:rounded-3xl overflow-hidden border border-yellow-600/30 p-6 sm:p-10 md:p-14"
          style={{ background: 'linear-gradient(135deg, #1A1200 0%, #2C1F00 50%, #1A1200 100%)' }}
        >
          <div
            className="absolute inset-0 opacity-20 pointer-events-none"
            style={{ backgroundImage: 'radial-gradient(circle at 20% 50%, #D4AF37 0%, transparent 60%), radial-gradient(circle at 80% 50%, #D4AF37 0%, transparent 60%)' }}
          />
          <div className="relative z-10 flex flex-col items-center text-center gap-4 sm:gap-5">
            <h3
              className="font-black text-white w-full"
              style={{ fontFamily: 'Montserrat, sans-serif', fontSize: 'clamp(1.2rem, 3.5vw, 2rem)', lineHeight: 1.2 }}
            >
              Ready to Place Your First Bet?
            </h3>
            <p className="text-gray-300 text-sm sm:text-base max-w-lg">
              Join over 3 million Indians who have already found their trusted home for online
              betting. Getting started is easier than you think.
            </p>
            <a
              href={WHATSAPP_URL}
              target="_blank"
              rel="noopener noreferrer"
              id="sports-section-cta"
              className="inline-flex items-center gap-2 gold-btn px-7 py-3.5 rounded-full text-sm sm:text-base font-black uppercase tracking-wider"
            >
              <WhatsAppIcon />
              Claim Your Betting ID Now
            </a>
          </div>
        </div>
      </div>
    </section>
  );
}

function WhatsAppIcon() {
  return (
    <svg className="w-4 h-4 sm:w-5 sm:h-5 flex-shrink-0" fill="currentColor" viewBox="0 0 448 512">
      <path d="M380.9 97.1C339 55.1 283.2 32 223.9 32c-122.4 0-222 99.6-222 222 0 39.1 10.2 77.3 29.6 111L0 480l117.7-30.9c32.4 17.7 68.9 27 106.1 27h.1c122.3 0 224.1-99.6 224.1-222 0-59.3-25.2-115-67.1-157zm-157 341.6c-33.2 0-65.7-8.9-94-25.7l-6.7-4-69.8 18.3L72 359.2l-4.4-7c-18.5-29.4-28.2-63.3-28.2-98.2 0-101.7 82.8-184.5 184.6-184.5 49.3 0 95.6 19.2 130.4 54.1 34.8 34.9 56.2 81.2 56.1 130.5 0 101.8-84.9 184.6-186.6 184.6zm101.2-138.2c-5.5-2.8-32.8-16.2-37.9-18-5.1-1.9-8.8-2.8-12.5 2.8-3.7 5.6-14.3 18-17.6 21.8-3.2 3.7-6.5 4.2-12 1.4-32.6-16.3-54-29.1-75.5-66-5.7-9.8 5.7-9.1 16.3-30.3 1.8-3.7.9-6.9-.5-9.7-1.4-2.8-12.5-30.1-17.1-41.2-4.5-10.8-9.1-9.3-12.5-9.5-3.2-.2-6.9-.2-10.6-.2-3.7 0-9.7 1.4-14.8 6.9-5.1 5.6-19.4 19-19.4 46.3 0 27.3 19.9 53.7 22.6 57.4 2.8 3.7 39.1 59.7 94.8 83.8 35.2 15.2 49 16.5 66.6 13.9 10.7-1.6 32.8-13.4 37.4-26.4 4.6-13 4.6-24.1 3.2-26.4-1.3-2.5-5-3.9-10.5-6.6z" />
    </svg>
  );
}
